...
# from scipy import misc  # 如果后面确实需要再打开
...
class deep_atari:
    def __init__(self, params):
        ...
        self.engine = emulator(rom_name='breakout.bin', vis=False)
        ...
        for numeps in range(self.params['num_episodes']):
            ...
            for maxl in range(self.params['episode_max_length']):
                if state_proc_old is not None:
                    self.DB.insert(state_proc_old[:, :, 3], reward, int(action), terminal)


                action = self.perceive(state_proc, terminal)
                if action is None:  # TODO - check [terminal condition]
                    break

                # 是否到达可学习阶段
                if (local_cnt > self.params['train_start'] and
                        local_cnt % self.params['learning_interval'] == 0):
                ...
                # 定期保存 checkpoint
                if (local_cnt > self.params['train_start'] and
                        local_cnt % self.params['save_interval'] == 0):
                ...
                # 执行一步游戏操作
                state, reward, terminal = self.engine.next(action)
                state_resized = cv2.resize(state, (84, 110))
                state_gray = cv2.cvtColor(state_resized, cv2.COLOR_BGR2GRAY)
                # 滑动帧
