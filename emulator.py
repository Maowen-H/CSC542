import numpy as np
import copy
import sys
from ale_py import ALEInterface
import cv2
import time
# 如果需要保存图像，推荐使用 imageio 或 PIL，而不是 scipy.misc
# import imageio

class emulator:
    def __init__(self, rom_name, vis):
        self.ale = ALEInterface()
        self.max_frames_per_episode = self.ale.getInt("max_num_frames_per_episode")
        self.ale.setInt("random_seed", 123)
        self.ale.setInt("frame_skip", 4)
        self.ale.loadROM('roms/' + rom_name)
        self.legal_actions = self.ale.getMinimalActionSet()
        self.action_map = dict()
        for i in range(len(self.legal_actions)):
            self.action_map[self.legal_actions[i]] = i

        self.screen_width, self.screen_height = self.ale.getScreenDims()
        print("width/height: " + str(self.screen_width) + "/" + str(self.screen_height))
        self.vis = vis
        if vis:
            cv2.startWindowThread()
            cv2.namedWindow("preview")

    def get_image(self):
        image = np.zeros((self.screen_width, self.screen_height, 3), dtype=np.uint8)
        self.ale.getScreenRGB(image)
        return image

    def newGame(self):
        self.ale.reset_game()
        return self.get_image()

    def next(self, action_indx):
        reward = self.ale.act(action_indx)
        nextstate = self.get_image()
        # 如果需要保存截图，建议使用 imageio.imwrite('test.png', nextstate) 
        # 而不是 scipy.misc.imsave，因为新版本 scipy 已经移除 misc.imsave

        if self.vis:
            cv2.imshow('preview', nextstate)
        return nextstate, reward, self.ale.game_over()


if __name__ == "__main__":
    engine = emulator('breakout.bin', True)
    engine.next(0)
    time.sleep(5)
